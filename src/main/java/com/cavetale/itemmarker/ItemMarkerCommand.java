...
final class ItemMarkerCommand implements CommandExecutor {
    ...
    final class ItemMarkerException extends Exception {
        ItemMarkerException(String message) {
            super(message);
        }
    }
    ...
    private boolean onCommand(Player player, String cmd, String[] args) throws ItemMarkerException {
        switch (cmd) {
        case "info": {
            ItemStack item = player.getInventory().getItemInHand();
            if (item == null) throw new ItemMarkerException("No item in hand!");
            player.sendMessage("Item=" + item.getType()
                               + " " + ItemMarker.ITEM_ID_KEY + "=" + ItemMarker.getCustomId(item)
                               + " " + ItemMarker.ITEM_OWNER_KEY + "=" + ItemMarker.getOwner(item));
            return true;
        }
